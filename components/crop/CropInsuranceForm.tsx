...
interface finalForm {
  ...
  phone: "";
  email: "";
  insurance_type_id: 5; // Hardcoded for crop insurance
...
const CropInsuranceForm = () => {
    ...
    const fetchFormFields = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/insurance/insurance-types/5/form/`
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      //  console.error('Error submitting crop insurance form:', error);
